...
coords = wmos.wcs_world2pix(mosaic_cat['RA'], mosaic_cat['DEC'], 1)
mosaic_cat.add_column(coords[0], name='X')
mosaic_cat.add_column(coords[1], name='Y')
...
for i,star in enumerate(mosaic_cat):
    x, y, = star['X']-1., star['Y']-1. # Subtract 1 because SExtractor coordinates start at 1.
    ...
    e1, e2, width, q11, q22, q12 = quadrupole(post, post.shape[0]//2, post.shape[1]//2, 2.) # Measure quadrupole moments using a 2. pixel sigma Gaussian weight.
    imax = np.unravel_index(np.argmax(post), post.shape) # Find brightest pixel.
    xcen, ycen = imax[1], imax[0]

    if (star['F160W'] > 18) & (star['F160W'] < 21) & (ycen == w) & (xcen == w):
...
keepers = (xcorr >= 0.) #& (e < 0.04) & (mosaic_cat['F160W'] > 18.5) & (mosaic_cat['F160W'] < 21)
...
if on:
    ...
    ratio = (good_stars['X'].max() - good_stars['X'].min()) / (good_stars['Y'].max() - good_stars['Y'].min())
    ...
    crosscorr = ax2.scatter(mosaic_cat['F160W'], xcorr, color='black')
    #ax2.scatter(star['F160W'], xc, color='red')
    ax2.set_title('Cross-correlation (r<5 pixels)')
    ax2.set_xlabel('F160W Mag')
    ax2.set_ylabel('X-corr')

    xp, yp = get_cartesian(mosaic_cat['X'], mosaic_cat['Y'], e*5000, phi)
    ...
    def update_imshow(ind):
        ...
        ax.axvline(w)
        ax.axhline(w)
        ax.text(0,28, 'F160W=%s' % round(star['F160W'],1), color='black')
        ax.text(0,26, 'e=%s' % round(e_[ind],3), color='black')
        ax.set_title('Postage Stamp ID: %s' % star['ID'])
    ...
    def update_xcorr(ind):
        ...
        star = mosaic_cat[ind]
        xcplot = ax2.scatter(star['F160W'], xcorr[ind], color='red', gid=100001)
        return xcplot
    ...
    def update_whisker(ind):
        ...
        for p in ps:
            if p.get_gid() == 100000:
                p.remove()
        star = mosaic_cat[ind]
        ell2 = Ellipse((star['X'], star['Y']),width=500*(1+e_[ind]*5),height=500, angle=pa[ind]*180/np.pi, facecolor='none', edgecolor='red', gid=100000)
...
xp, yp = get_cartesian(good_stars['X'], good_stars['Y'], e*8000, phi)
